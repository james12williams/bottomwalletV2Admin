...
static clearAuthUserData(reload=false){
...
static allRequest(requests){
    let authFetch: axios.AxiosInstance;
    ...
    for (i = form.elements.length - 1; i >= 0; i = i - 1) {
        ...
        switch (form.elements[i].nodeName) {
            case 'INPUT':
                ...
                break;
            case 'TEXTAREA':
                data[form.elements[i].name] = form.elements[i].defaultValue;
                break;
            case 'SELECT':
                switch (form.elements[i].type) {
                    case 'select-one':
...
static removeEmptyValues (obj:array){
    let value = {};
...
static listen () {
    const options = {
        broadcaster: 'pusher',
        key: 'as40505dspo595spyhu5',
        cluster: 'mt1',
        forceTLS: import.meta.env.VITE_APP_PUSHER_TLS,
        authEndpoint: import.meta.env.VITE_APP_PUSHER_AUTH_HOST,
        auth: {
            headers: {
