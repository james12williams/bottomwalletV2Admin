...
var KTApp = function () {
    ...
    var createDateRangePickers = function() {
        ...
        elements.map(function (element) {
            ...
            input_from.setAttribute('type', 'hidden');
            input_from.name = element.getAttribute('data-name')+'_from';
            element.appendChild(input_from);
            const input_to = document.createElement("input");
            input_to.setAttribute('type', 'hidden');
            input_to.name = element.getAttribute('data-name')+'_to';
            element.appendChild(input_to);
            ...
            var cb = function(start, end) {
                ...
                if (display) {
                    if ( current.isSame(start, "day") && current.isSame(end, "day") ) {
                        ...
                        input.value = start.format('DD-MM-YYYY') + ' - ' + end.format('D-m-YYYY');
                    }
                    input_from.value = start.format('DD-MM-YYYY');
                    input_to.value = end.format('DD-MM-YYYY');
                }
            }
    ...
    const createQuill = function (){
        const elements = [].slice.call(document.querySelectorAll('[data-control="quill"], [data-kt-quill="true"]'));
        ...
        elements.map(function (element) {
            ...
            let toolbar = element.getAttribute("data-kt-toolbar");
            let placeholder = element.getAttribute("data-placeholder");
            let toolbarContainer = element.getAttribute("data-toolbar-container");
            ...
            if (toolbarContainer){
                var checktoolbarContainer = document.getElementById(toolbarContainer)
                if (checktoolbarContainer) {
                    config.toolbar = {container: "#" + toolbarContainer}
                }
            }
            ...
            const quill = new Quill(element, {
                modules: config,
                placeholder: placeholder?placeholder:'Type your text here...',
            ...
            if (element.nodeName != 'INPUT' || element.nodeName != 'TEXTAREA') {
                ...
                input.name = element.getAttribute('data-name');
                element.appendChild(input);
                input.innerHTML = quill.getSemanticHTML();
    ...
    const createDatepicker = function (){
        const elements = [].slice.call(document.querySelectorAll('[data-control="flatpickr"], [data-kt-flatpickr="true"]'));
        ...
        elements.map(function (element) {
            ...
            let config = {}
            let enableTime = element.getAttribute("data-enable-time")
            let multiple = element.getAttribute("data-multiple")
            let noCalendar = element.getAttribute("data-disable-calendar")
            if (enableTime || noCalendar){
                config.enableTime = enableTime;
                if (noCalendar){
                    config.noCalendar = noCalendar;
                }
            }
            if (multiple){
    ...
    const createUiSlider = function (){
        const elements = [].slice.call(document.querySelectorAll('[data-control="uiSlider"], [data-kt-ui-slider="true"]'));
        ...
        elements.map(function (element) {
            ...
            var slider = document.querySelector("#"+element.getAttribute("data-slider"));
            var value = document.querySelector("#"+element.getAttribute("data-label"));
            var max = element.getAttribute("max");
            if (!max){
                max = 100;
            }
            var min = element.getAttribute("min", 10);
            if (!min){
                min = 0;
            }
            var step = element.getAttribute("step", 10);
            if (!min){
                step = 1;
            }
            var defaultValue = element.getAttribute("value", 0);
    ...
    const createDropzone = function (){
        const elements = [].slice.call(document.querySelectorAll('[data-control="dropzone"], [data-kt-dropzone="true"]'));
    ...
    var createTagify = function () {
        // Check if jQuery included
        if (typeof jQuery == 'undefined') {
            return;
        }

        var elements = [].slice.call(document.querySelectorAll('[data-control="tagify"], [data-kt-tagify="true"]'));
        ...
        elements.map(function (element) {
            ...
            let whitelist = [];
            if (element.getAttribute('data-whitelist')) {
                whitelist = JSON.parse(element.getAttribute('data-whitelist'));
            }
    ...
    var createStatusIndicator = function () {
        ...
        var elements = [].slice.call(document.querySelectorAll('[data-kt-indicator="true"]'));

        elements.map(function (element) {
            if (element.getAttribute("data-kt-initialized-status-indicator") === "1") {
                return;
            }
            let indicatorId = element.getAttribute('data-indicator');
            const indicator = document.getElementById(indicatorId)
            if (indicator){
                let options = [];
                if (indicator.getAttribute('data-indicator-options')) {
                    options = JSON.parse(indicator.getAttribute('data-indicator-options'));
                }
                $(element).on('change', function (e) {
                    const value = e.target.value;
                    const optionVal = options[value];
                    indicator.classList.remove(...options);
                    indicator.classList.add(optionVal);
                });
                const optionVal = options[element.value];
                indicator.classList.remove(...options);
                indicator.classList.add(optionVal);
                element.setAttribute("data-kt-initialized-status-indicator", "1");
            }
        });
    }
    ...
    var hideElements = function (elements) {
        if ((typeof elements) == 'string'){
            ...
            const element_container = document.getElementById(elements+'_container');
            if (element_container) {
                element_container.classList.add('d-none');
            }
        }
        else if (elements){
            [].slice.call(elements).map(function (elementId) {
                const element = document.getElementById(elementId);
                if (element) {
                    element.parentNode.classList.add('d-none');
                }
                const element_container = document.getElementById(elementId+'_container');
                if (element_container) {
                    element_container.classList.add('d-none');
                }
            });
    ...
    var showElements = function (elements) {
        if ((typeof elements) == 'string'){
            ...
            const element_container = document.getElementById(elements+'_container');
            if (element_container) {
                element_container.classList.remove('d-none');
            }
        }
        else if (elements){
            [].slice.call(elements).map(function (elementId) {
    ...
    var createHidables = function () {
        ...
        elements.map(function (element) {
            if (element.getAttribute("data-kt-initialized-hiders") === "1") {
                return;
            }
            let toHide = element.getAttribute('data-to-hide');
            if (toHide){
                toHide = JSON.parse(toHide);
            }
            let toShow = element.getAttribute('data-to-show');
            if (toShow){
            ...
            if (toHide || toShow){
                $(element).on('change', function (e) {
                    const value = e.target.value;
                    hideElements(toHide[value])
                    showElements(toShow[value])
                });
                hideElements(toHide[element.value])
                showElements(toShow[element.value])
                element.setAttribute("data-kt-initialized-hiders", "1");
            }
        });
    }
